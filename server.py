...
def run_inference(density_3d: np.ndarray, vf_target: float):
    ...
    try:
        ...
        max_strength = float(np.max(mean_curve))
        a_basis = max_strength * 0.84
        b_basis = max_strength * 0.91
        return {
            "strain": [float(f"{s:.6f}") for s in strain],
            "stress": [float(f"{t:.2f}") for t in mean_curve],
            "stress_std": [float(f"{s:.2f}") for s in std_curve],
            "a_basis": round(a_basis, 2),
            "b_basis": round(b_basis, 2),
            "max_strength": float(np.max(mean_curve)),
            "vf_used": vf_target,
            "uncertainty": True
        }
...
def predict_properties_fallback(vf_target: float):
    ...
    cutoff = min(len(stress), peak_idx + int(0.05 * len(stress)))
...
@app.post("/predict/")
async def predict(vf_target: float = Form(None), slice_a: UploadFile = File(None), slice_b: UploadFile = File(None)):
    ...
    try:
        if vf_target is not None:
            ...
            vf = vf_target
        elif slice_a and slice_b and slice_a.filename and slice_b.filename:
            for name, file in [("slice_a", slice_a), ("slice_b", slice_b)]:
                with open(os.path.join(job_dir, f"{name}.png"), "wb") as f:
                    f.write(await file.read())
            vf = 60.0
        else:
            raise HTTPException(400, "Укажите Vf или загрузите два среза")
        
        gen = FiberGenerator(FIBER_RADIUS, vf, max_pol=3.0, az_var=15.0)
        ...
        result = run_inference(density, vf)
